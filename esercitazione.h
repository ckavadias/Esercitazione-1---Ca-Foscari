...
double previous_departure[MAX_LINES];
int test_num = 0;
int average_users = 0;

/* function prototypes */
void initialise(int situation);
void departure(double velocity);
void response();
...
void initialise(int situation){
	FILE* poisson_list;
	int num_entries = 0, event_num, line = situation, i;
	double storage = 0.00;

 /*reset simulation variables */
 	for( i = 0; i < MAX_LINES; i++){
 		previous_departure[i] = 0.00;
 	}
	sim_time = 0.00;
	size_list = 1000;
 ...
 /* store arrival times and assign execution line*/
	...
	/* assign line numbers based on situation to be simulated */	
	switch (situation){
	case 1:
		event_list[num_entries].line = 1;
		break;
	case 2:
		event_list[num_entries].line = line%2 + 1;
		line++;
		break;
	case 3:
		event_list[num_entries].line = rand()%2 + 1;
		break;
	}
		num_entries++;
	}
...
void departure(double velocity){
	...
	for ( i = 0; i < size_list; i++){
		line = event_list[i].line - 1;
		
		if ( !previous_departure[line]){
			sim_time = event_list[i].arrival;
		}
		else if( event_list[i].arrival < previous_departure[line] ){
			sim_time = previous_departure[line];
		}
		else{
			sim_time = event_list[i].arrival;
		}
		event_list[i].departure = sim_time + exponential(velocity);
		previous_departure[line] = event_list[i].departure;
	}
}
